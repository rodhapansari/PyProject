...
while True:
    ...
    for face in faces:
        ...
        for n in range(0,68):
                ...
                cv2.putText(frame,str("Happy Birthday To You Tanish"), (x,y), cv2.FONT_HERSHEY_SIMPLEX, 0.5, 255)
        ...
        if(abs(x2-x1)>20):
                print("in if")
                nose = imutils.resize(nose, height=abs(x2-x1),width=abs(x2-x1))
                mask = imutils.resize(a, height=abs(x2-x1),width=abs(x2-x1))
        else:
                print("in else")
